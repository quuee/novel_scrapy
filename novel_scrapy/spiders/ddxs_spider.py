...
class DdxsSpider(scrapy.Spider):
    # name属性：必须且唯一
    name = "ddxs" 
    start_urls = ["https://www.ddxs.com/zhutianwanjiezhidazhengjiu/"]
    # allowed_domains = []
    # custom_settings = {}

    def parse(self, response):
        print(response)
        # el_list = response.xpath("//table[@id='at']/tbody/tr/td[@class='L']/a")
        # for el in el_list:
    ...
    def parse_chapter(self, response):
        title = response.xpath("//dd/h1/text()").get()
        txt = response.xpath("//dd[@id='contents']/text()").getall()
